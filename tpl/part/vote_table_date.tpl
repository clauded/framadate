        ...
        <table class="results">
            ...
            {if $active && $editingVoteId === 0 && !$expired && $accessGranted}
                <tr id="vote-form" class="hidden-print">
                    ...
                    {/foreach}
                    <td>
                        <button type="submit" class="btn btn-default btn-xs btn-success" name="save" title="{__('Poll results', 'Save the choices')} {$vote->name|html}">
                    ...
                    {foreach $slots as $slot}
                        {foreach $slot->moments as $moment}
                            {if $best_choices['y'][$i] == $max}
                                {assign var="space" value="`$slot->day|intl_date_format:'d-m-Y'|html`|`$moment`"}
