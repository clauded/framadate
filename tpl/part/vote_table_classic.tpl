    ...
    {if $max > 0}
                ...
                <tr id="total">
                    <td class="text-left">
                        ...
                        {$votes|count} 
                        {if ($votes|count)==1}{__('Poll results', 'polled user')}
                        {else}{__('Poll results', 'polled users')}
                        {/if}
                        {if $poll->ValueMax ne NULL && ($votes|count)>=$poll->ValueMax} ({__('Generic', 'maximum voters reached')}!)
                        {elseif $poll->ValueMax ne NULL && $poll->ValueMax > 0} ({__('Generic', 'maximum')}: {$poll->ValueMax})
                        {/if}
                    </td>
	...
	{if $poll->ValueMax eq NULL || $votes|count lt $poll->ValueMax}
        ...
        {foreach $slots as $id=>$slot}
    		...
    		{if $poll->value_max eq NULL || $best_choices['y'][$id] lt $poll->value_max}{$class = 'yes'}{else}{$class = 'hidden'}{/if}
                       		<li class="{$class}">
                            	<input type="radio" id="y-choice-{$id}" name="choices[{$id}]" value="2" {(!isset($selectedNewVotes[$id]) || ("2" !== $selectedNewVotes[$id])) ? "" : " checked"} />
			...
			{$class = 'no hidden'}
                            <li class="{$class}">
                                ...
                                <input type="radio" id="n-choice-{$id}" name="choices[{$id}]" value=" "	{(!isset($selectedNewVotes[$id]) || ("" !== $selectedNewVotes[$id])) ? "" : " checked"}	/>
                            ...
                            <li class="hide">
                                <input type="radio" id="r-choice-{$id}" name="choices[{$id}]" value=" " {(isset($selectedNewVotes[$id]) && ("" !== $selectedNewVotes[$id])) ? "" : " checked"} />
                            </li>
                        </ul>
                    </td>
    		{$id = $id+1}
        {/foreach}
                    <td>
                        <button type="submit" class="btn btn-default btn-xs btn-success" name="save" title="{__('Poll results', 'Save the choices')}">
                            <i class="glyphicon glyphicon-save" aria-hidden="true"></i><span class="sr-only">{__('Generic', 'Save')}</span>
                        </button>
                    </td>
                </tr>
    {/if}
