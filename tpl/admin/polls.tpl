                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="poll" id="poll" class="form-control" value="{$search['poll']|html}"/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="title" class="control-label">{__('Admin', 'Title')}</label>
                            <input type="text" name="title" id="title" class="form-control" value="{$search['title']|html}"/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="name" class="control-label">{__('Admin', 'Author')}</label>
                            <input type="text" name="name" id="name" class="form-control" value="{$search['name']|html}"/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="mail" class="control-label">{__('Admin', 'Email')}</label>
                            <input type="text" name="mail" id="mail" class="form-control" value="{$search['mail']|html}"/>
                        </div>
                    </div>
                </div>
            ...
            <div class="table-of-polls panel">
                <table class="table table-polls">
                    <tr align="left">
                        ...
                        <th scope="col" colspan="3" class="text-center">{__('Admin', 'Actions')}</th>
                    </tr>
{foreach $polls as $poll}
                        <tr align="left">
                            <td class="cell-format" align="center">
    {if $poll->format === 'D'}
                                <span class="glyphicon glyphicon-calendar" aria-hidden="true" title="{__('Generic', 'Date')}"></span>
                                <span class="sr-only">{__('Generic', 'Date')}</span>
    {else}
                                <span class="glyphicon glyphicon-list-alt" aria-hidden="true" title="{__('Generic', 'Classic')}"></span>
                                <span class="sr-only">{__('Generic', 'Classic')}</span>
    {/if}
                            </td>
                            <td class="cell-format">{$poll->title|html}</td>
                            <td class="cell-format">{$poll->admin_name|html}</td>
                            <td class="cell-format">{$poll->admin_mail|html}</td>
    {if strtotime($poll->end_date) > time()}
                            <td class="cell-format">{date('d/m/y', strtotime($poll->end_date))}</td>
    {else}
                            <td class="cell-format">
                                <span class="text-danger">{strtotime($poll->end_date)|date_format:'d/m/Y'}</span>
                            </td>
    {/if}
                            <td class="cell-format">{$poll->votes|html}</td>
                            <td class="cell-format">{$poll->id|html}</td>
                            <td class="cell-format" align="center">
                                <a href="{poll_url id=$poll->id}" class="btn btn-link" title="{__('Admin', 'See the poll')}">
                                    <span class="glyphicon glyphicon-eye-open"></span>
                                    <span class="sr-only">{__('Admin', 'See the poll')}</span>
                                </a>
                            </td>
                            <td class="cell-format" align="center">
                                <a href="{poll_url id=$poll->admin_id admin=true}" class="btn btn-link" title="{__('Admin', 'Change the poll')}">
...
{for $p=1 to $pages}
    {if $p===$page}
                    <a href="{$SERVER_URL}admin/polls.php?page={$p}&{$search_query}" class="btn btn-danger" disabled="disabled">{$p}</a>
    {else}
