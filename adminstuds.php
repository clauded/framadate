...
if (isset($_POST['update_poll_info'])) {
    $updated = false;
    $field = $inputService->filterAllowedValues($_POST['update_poll_info'], ['title', 'admin_mail', 'description', 'rules', 'expiration_date', 'ValueMax', 'value_max', 'name', 'hidden', 'removePassword', 'password']);
    ...
    } elseif ($field === 'ValueMax') {
        $ValueMax = $inputService->filterValueMax($_POST['ValueMax']);
        if ($ValueMax) {
            $poll->ValueMax = $ValueMax;
            $updated = true;
        } else {
            $poll->ValueMax = NULL;
            $updated = true;
        }
    } elseif ($field === 'value_max') {
        $value_max = $inputService->filtervalue_max($_POST['value_max']);
        if ($value_max) {
            $poll->value_max = $value_max;
            $updated = true;
        } else {
            $poll->value_max = NULL;
            $updated = true;
